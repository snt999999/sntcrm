...
const SMS_PROCESS_LIMIT = 25;
const SMS_SENDING_STUCK_MINUTES = 15;

function checkCronAuth(request, env, source = "http") {
  if (source === "scheduled") return { ok: true, actor: "cloudflare-cron" };
  const secret = String(env.SMS_CRON_SECRET || "").trim();
  const url = new URL(request.url);
  const bySecret = request.headers.get("x-cron-secret") || url.searchParams.get("secret") || "";
  if (secret && bySecret === secret) return { ok: true, actor: "cron-secret" };

  // Для ручной кнопки в админке: полный админ может запустить очередь без знания SMS_CRON_SECRET.
  const adminPassword = String(env.ADMIN_PASSWORD || "").trim();
  const byAdmin = String(request.headers.get("x-admin-password") || "").trim();
  if (adminPassword && byAdmin === adminPassword) return { ok: true, actor: "admin" };

  if (!secret) return { ok: false, status: 500, body: { ok: false, error: "SMS_CRON_SECRET is not set. Задайте SMS_CRON_SECRET в Cloudflare и в cron-worker." } };
  return { ok: false, status: 401, body: { ok: false, error: "Неверный SMS_CRON_SECRET" } };
}

async function resetStuckSending(env) {
  const cutoff = new Date(Date.now() - SMS_SENDING_STUCK_MINUTES * 60 * 1000).toISOString();
  try {
    const data = await sbFetch(env, `/rest/v1/sms_queue?status=eq.sending&updated_at=lt.${encodeURIComponent(cutoff)}&select=*`, {
      method: "PATCH",
      headers: { "Prefer": "return=representation" },
      body: JSON.stringify({ status: "queued", error: "Повторная постановка в очередь после зависшего статуса sending" })
    });
    return asArr(data).length;
  } catch (_) {
    return 0;
  }
}

async function listDueSms(env) {
  const now = new Date().toISOString();
  const data = await sbFetch(env, `/rest/v1/sms_queue?select=*&status=eq.queued&manual_only=eq.false&send_at=lte.${encodeURIComponent(now)}&order=send_at.asc&limit=${DEFAULT_SMS_LIMIT}`);
  return asArr(data);
}

async function claimSmsRecord(env, record) {
  // Защита от дублей: отправлять можно только если запись всё ещё queued.
  // Если второй cron/клик уже забрал SMS и поставил sending, этот PATCH вернёт пустой массив.
  const meta = {
    ...(record.meta || {}),
    cron_claimed_at: new Date().toISOString(),
    cron_claim_note: "claimed before sending to prevent duplicate SMS"
  };
  const data = await sbFetch(env, `/rest/v1/sms_queue?id=eq.${encodeURIComponent(record.id)}&status=eq.queued&select=*`, {
    method: "PATCH",
    headers: { "Prefer": "return=representation" },
    body: JSON.stringify({ status: "sending", error: "", meta })
  });
  return asArr(data)[0] || null;
}

async function patchSmsStatus(env, id, patch, metaPatch = {}) {
  const existingData = await sbFetch(env, `/rest/v1/sms_queue?select=*&id=eq.${encodeURIComponent(id)}&limit=1`);
  const existing = asArr(existingData)[0] || {};
  const meta = { ...(existing.meta || {}), ...metaPatch };
  const data = await sbFetch(env, `/rest/v1/sms_queue?id=eq.${encodeURIComponent(id)}&select=*`, {
    method: "PATCH",
    headers: { "Prefer": "return=representation" },
...
async function processOneSms(env, record) {
  const claimed = await claimSmsRecord(env, record);
  if (!claimed) return { id: record.id, ok: true, skipped: true, reason: "Уже забрано другим запуском cron" };

  const to = normalizePhone(claimed.phone || "");
  const message = await approvedSmsMessageForQueue(env, claimed);
  if (!to || !message) {
    const error = !to ? "Нет телефона" : "Текст SMS не совпадает с утверждёнными шаблонами SIGMA";
    await patchSmsStatus(env, claimed.id, { status: "failed", error }, { status_checked_at: new Date().toISOString() });
    return { id: claimed.id, ok: false, error };
  }

  const sent = await sendSigmaSms(env, to, message);
  if (sent.ok) {
    await patchSmsStatus(env, claimed.id, {
      status: "sent",
      sent_at: new Date().toISOString(),
      provider_message_id: sent.smsId || "",
      error: "",
      message
    }, {
      delivery_status: sent.status || "created",
      service_response: compactJson(sent.result || sent),
      cost: String(sent.cost ?? ""),
      balance: String(sent.balance ?? ""),
      status_checked_at: new Date().toISOString(),
      sigma_approved_template_text: message,
      cron_sent_at: new Date().toISOString()
    });
  } else {
    await patchSmsStatus(env, claimed.id, {
      status: "failed",
      error: sent.error || "Ошибка отправки",
      message
    }, {
      delivery_status: sent.status || "ERROR",
      service_response: compactJson(sent.result || sent),
      status_checked_at: new Date().toISOString(),
      sigma_approved_template_text: message,
...
async function runSmsCronCore({ env, source = "http", actor = "cron" }) {
  const reset = await resetStuckSending(env);
  const due = await listDueSms(env);
  const results = [];
  for (const record of due.slice(0, SMS_PROCESS_LIMIT)) {
    results.push(await processOneSms(env, record));
  }
  return {
    ok: true,
    provider: "supabase+sigma",
    source,
    actor,
    resetStuckSending: reset,
    checked: due.length,
    due: due.length,
...
async function runSmsCron({ request, env, source }) {
  const auth = checkCronAuth(request, env, source);
  if (!auth.ok) return json(auth.body, auth.status);
  try {
    const result = await runSmsCronCore({ env, source, actor: auth.actor });
    return json(result);
  } catch (error) {
    return json({ ok: false, provider: "supabase", error: error.message, supabaseResponse: error.response || null }, error.status || 500);
  }
}

// На случай запуска через Cloudflare Scheduled Handler в окружениях, где он поддерживается.
export async function onScheduled(event, env, ctx) {
  ctx.waitUntil(runSmsCronCore({ env, source: "scheduled", actor: "cloudflare-cron" }));
}

export async function onRequest(context) {
  if (context.request.method === "GET") return onRequestGet({ request: context.request, env: context.env });
  if (context.request.method === "POST") return onRequestPost({ request: context.request, env: context.env });
