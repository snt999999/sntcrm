...
async function listDueSms(env) { const now = new Date().toISOString(); const data = await sbFetch(env, `/rest/v1/sms_queue?select=*&status=eq.queued&send_at=lte.${encodeURIComponent(now)}&order=send_at.asc&limit=${DEFAULT_SMS_LIMIT}`); return asArr(data); }
